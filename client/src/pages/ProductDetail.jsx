...
const ProductDetail = () => {
  ...
  const { getSingleProduct, products } = productContextValue();
  ...
  useEffect(() => {
    ...
    const categoryRel = singleProduct?.category?.toLowerCase();
    const realtedProd = products?.filter(
      (prod) => prod?.category?.toLowerCase() === categoryRel
    );
    setRelatedProducts(realtedProd);
  ...
  return (
    <>
      ...
      <div className="custom-container  my-10">
        <h2 className="text-2xl text-center underline underline-offset-8 font-semibold text-primary">
          Related Products{" "}
        </h2>
